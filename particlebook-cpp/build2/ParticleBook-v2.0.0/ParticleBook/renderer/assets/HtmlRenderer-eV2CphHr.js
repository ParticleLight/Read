...
function highlightTextInDOM(container, query) {
  const marks = [];
  ...
  for (const node of textNodes) {
    ...
    let lastIdx = 0;
    while (idx !== -1) {
      fragments.push(text2.slice(lastIdx, idx));
      const mark = document.createElement("mark");
      mark.style.backgroundColor = "rgba(251,191,36,0.3)";
      mark.style.borderRadius = "2px";
      mark.style.color = "inherit";
      mark.textContent = text2.slice(idx, idx + query.length);
      fragments.push(mark);
      marks.push(mark);
    ...
    if (lastIdx < text2.length) {
      fragments.push(text2.slice(lastIdx));
    }
...
function HtmlRenderer({ book, content, bookId }) {
  ...
  const searchMarksRef = reactExports.useRef([]);
